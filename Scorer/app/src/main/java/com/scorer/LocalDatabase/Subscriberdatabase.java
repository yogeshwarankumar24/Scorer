...
public class Subscriberdatabase {
    ...
    public Subscriberdatabase(Activity context)
    {
        try
        {
            sqldb_message = "";
    ...
    public void CreateDatabase()
    {
        try
        {
            ...
            if (!Exists())
            {
                ...
                sqldb_query = "CREATE TABLE IF NOT EXISTS Subscriberdatabase (uniqueid string,gameid string,gamename string,playername string,playeremail string,playerposition string,status string,playerimage BYTE,PRIMARY KEY(uniqueid,uniqueid));";
    ...
    public boolean Exists()
    {
        try
        {
            android.database.Cursor cursor = sqldb.rawQuery("select * from Subscriberdatabase", null);
            return true;
        }
        catch (SQLException ex)
        {
    ...
    public void AddRecord(PlayerModel objabout)
    {
        try
        {
            ...
            sqldb.insert("Subscriberdatabase", null, insertValuesy);
        }
        catch (SQLException ex)
        {
            sqldb_message = ex.getMessage();
    ...
    public void DeleteRecord(String uniqueid)
    {
        try
        {
            android.database.Cursor cursor = sqldb.rawQuery("select * from Subscriberdatabase where uniqueid='"+uniqueid+"'", null);
            if (cursor != null)
            {
                sqldb.delete("Subscriberdatabase", "uniqueid=?", new String[] {uniqueid});
    ...
    public void DeleteRecord()
    {
        try
        {
            ...
            if (cursor != null)
            {
                sqldb.delete("Subscriberdatabase", "uniqueid=?", new String[] {""});
                sqldb_message = "Record Deleted";
            }
        }
        catch (NullPointerException ex)
    ...
    public List<PlayerModel> GetRecords()
    {
        ...
        try
        {
            ...
            if(Common.objPlayers.size()>0)
            {
                ...
                cursor = sqldb.rawQuery("select * from Subscriberdatabase where uniqueid NOT IN ("+value+")", null);
            }
            else
            {
                cursor = sqldb.rawQuery("select * from Subscriberdatabase", null);
            }
//            android.database.Cursor cursor = sqldb.rawQuery("select * from Subscriberdatabase", null);
            if (cursor != null)
            {
    ...
    public List<PlayerModel> GetRecords(String Gameid)
    {
        ...
        try
        {
            ...
            if(Common.objPlayers.size()>0)
            {
                ...
                cursor = sqldb.rawQuery("select * from Subscriberdatabase where uniqueid NOT IN ("+value+") AND gameid='"+Gameid+"'", null);
            }
            else
            {
                cursor = sqldb.rawQuery("select * from Subscriberdatabase where gameid='"+Gameid+"'", null);
            }
            if (cursor != null)
            {
                while(cursor.moveToNext()) {
    ...
    public PlayerModel GetRecords_id(String Gameid)
    {
        ...
        try
        {
            android.database.Cursor cursor = sqldb.rawQuery("select * from Subscriberdatabase where uniqueid='"+Gameid+"'", null);
            if (cursor != null)
            {
                cursor.moveToFirst();
                objPlayerModel.setUniqueid(cursor.getString(cursor.getColumnIndex("uniqueid")));
